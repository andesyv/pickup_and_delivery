...
SolutionCached genRandSolutionCached(const Problem& p, std::default_random_engine& ran) {
    SolutionCached routes;
    const auto vSize{p.vehicles.size()};
    routes.resize(vSize+1);
    ...
    while (!remainingCalls.empty()) {
        routes.at(ran() % (vSize+1)).calls.push_back(remainingCalls.back());
        remainingCalls.pop_back();
    }

    for (auto& route : routes) {
        route.calls.insert(route.calls.end(), route.calls.begin(), route.calls.end());
        // Reshuffle each route after duplicating:
        std::shuffle(route.calls.begin(), route.calls.end(), ran);
    }
...
SolutionCached adaptiveCachedSearch(const Problem& p, std::default_random_engine& ran) {
    ...
    const auto accept = [&](const auto& cost) {
        // const auto p = temperature * cost / localBestCost;
        const auto p = 1.0 / std::exp(std::pow(localBestCost / static_cast<double>(cost - localBestCost), 0.1));
        // return p < rand() * temperature;
        return cost < localBestCost /*|| rand() < temperature*/;
    };
